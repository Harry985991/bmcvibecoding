  ...
  $$('.tab').forEach(tab=>{
    tab.addEventListener('click', ()=>{
      ...
      if(tab.dataset.target==='#view-period-return'){ renderPeriodicReturnDashboard(calculatePortfolioSummary(), { forceQuote: true }); }
      if(tab.dataset.target==='#view-watchlist'){ renderWatchlist(); }
      if(tab.dataset.target==='#view-market-hub'){ renderMarketHub(); }
  ...
  const handleRefreshAllClick = async (e)=>{
    ...
    try{
      ...
      const tasks = DB.stocks.map(async (s) => {
        ...
        if (q && !q.error && typeof q.price === 'number') {
          ...
          s.price = q.price;
          s.lastPriceAt = Number.isFinite(incomingTimeMs) ? new Date(incomingTimeMs).toISOString() : nowISO();
          successCount++;
      ...
      showBackupStatus(
        isAutoRefresh
          ? `自動更新報價完成，正在校準 ${heldSymbols.length || stockSymbols.length} 檔技術位置…`
          : `股價已更新 ${successCount} 個標的，正在校準技術位置…`
      );
      await refreshIndicatorsForAll({
        symbols: heldSymbols.length ? heldSymbols : stockSymbols,
        force: true
      });
    } catch(err) {
    ...
    if (!isAutoRefresh) {
      if ((failedSymbols.length > 0 || staleSymbols.length > 0) && successCount === 0) {
        ...
        showBackupStatus(`股價與技術位置已更新 ${successCount} 個標的 ✓`);
      }
  ...
  function refreshPortfolioViews(options = {}){
    ...
    if(overview && summary){
      ...
      runRefreshStep('renderTreemap', ()=>renderTreemap(summary));
      runRefreshStep('renderAllocation', ()=>renderAllocation(summary));
    }
    if(holdings && summary){
      runRefreshStep('renderHoldings', ()=>renderHoldings(summary));
      runRefreshStep('renderLongTermMetricsIfOpen', ()=>renderLongTermMetricsIfOpen(summary));
    }
    if(txns && summary){
  ...
  function fullRender(){
    ...
    renderLongTermMetricsIfOpen(summary);
    renderTxns(summary);
    renderAccounts();
    renderAllocation(summary);
