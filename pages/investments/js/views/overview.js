  ...
  function renderOverview(summary = calculatePortfolioSummary()){
    ...
    const tierDonutWrapEl = $('#overview-tier-donut-wrap');
    const tierDonutLeg = $('#overview-tier-donut-legend');
    const alertEl = $('#overview-alert');
    if(!kpiRow || !barsEl || !donutWrapEl || !donutLeg || !tierDonutWrapEl || !tierDonutLeg || !alertEl) return;
    ...
    kpiRow.innerHTML = `
      ...
      <div class="overview-kpi-card">
        <div class="overview-kpi-label">持倉市值</div>
    ...
    const ovSlices = ovClassDef.map(c => ({ ...c, value: ovByClass[c.key], pct: ovTotal > 0 ? ovByClass[c.key] / ovTotal : 0 })).filter(s => s.value > 0);
    ...
    // 分層比例（SVG donut）
    const tierDef = [
      { key: 'core',      label: '核心', color: '#0F766E' },
      { key: 'satellite', label: '衛星', color: '#2563EB' },
      { key: 'flex',      label: '偵查', color: '#D97706' },
    ];
    const tierBuckets = { core: 0, satellite: 0, flex: 0 };
    for(const row of summary.heldRows){
      const mv = parseN(row.marketValue);
      if(mv <= 0) continue;
      const tier = normalizeTierValue(getStockLabel(row.stock.id)?.tier);
      tierBuckets[tier] = (tierBuckets[tier] || 0) + mv;
    }
    const tierTotal = tierBuckets.core + tierBuckets.satellite + tierBuckets.flex;
    const tierSlices = tierDef.map(c => ({ ...c, value: tierBuckets[c.key], pct: tierTotal > 0 ? tierBuckets[c.key] / tierTotal : 0 })).filter(s => s.value > 0);

    tierDonutWrapEl.innerHTML = '<div class="overview-donut-center-label">分層比例</div>';
    tierDonutWrapEl.insertBefore(buildSvgDonut(tierSlices), tierDonutWrapEl.firstChild);
    tierDonutLeg.innerHTML = buildDonutLegend(tierSlices);
  ...
  function week1RenderTreemapHybrid(container, metrics, totalMV){
    ...
    const mainHtml = layoutRects.map((item) => {
      if(item.kind === 'side'){
        ...
        const showName = item.h >= 16;
        const showReturn = item.h >= 10;
        ...
        return `
          <button
            ...
            ${showName ? `<div style="font-size:10px;opacity:.8;line-height:1.05;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%">${week1EscapeHtml(row.stock?.name || '')}</div>` : ''}
            ${showReturn ? `<div style="font-size:${returnFontSize}px;font-weight:800;line-height:1">${sign}${(returnPct * 100).toFixed(1)}%</div>` : ''}
          </button>`;
      }
      ...
      week1OverviewTreemapChart = Highcharts.chart('overview-treemap', {
        ...
        series: [{
          ...
          dataLabels: {
            ...
            formatter: function () {
              ...
              const alloc = (this.point.custom.allocation * 100).toFixed(1);
              const mv = Math.round(this.point.value).toLocaleString();
              return `<div style="font-size:16px;font-weight:800;line-height:1.1">${this.point.name}</div>`
                + `<div style="margin-top:4px;font-size:12px;opacity:.82">${this.point.custom.stockName}</div>`
                + `<div style="margin-top:8px;font-size:15px;font-weight:800">${sign}${pct}%</div>`
                + `<div style="font-size:11px;opacity:.92">${mv} / ${alloc}%</div>`;
            }
          },
          point: {
            events: {
        ...
        tooltip: {
          ...
          formatter: function () {
            ...
            return `${this.point.name} ${c.stockName}<br>含息報酬率：${sign}${pct}%<br>市值：${Math.round(this.point.value).toLocaleString()}<br>配置：${alloc}%<br>含息損益：${Math.round(c.totalPnl).toLocaleString()}`;
          }
        }
      });
    }).catch((err) => {
