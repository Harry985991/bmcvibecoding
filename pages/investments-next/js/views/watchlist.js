  ...
  // ========= 預約單彙總（供現金治理 / 行動面板 / 決策資料包使用）=========
  // 依 V2.1「預約單全成交假設」：觀察中 / 今日關注的買進計畫一律視為可能成交。
  // 金額口徑與筆記卡一致：planPrice × planQty(張) × 1000 股。
  function getReservationSummary(){
    ensureWatchlist();
    const active = DB.watchlist.filter(item =>
      item && item.status !== 'done' && item.planPrice != null && item.planQty != null
    );
    const items = active.map(item => {
      const side = normalizeWatchSide(item.side);
      const amount = parseN(item.planPrice) * parseN(item.planQty) * 1000;
      return {
        symbol: item.symbol,
        name: item.name || '',
        side,
        status: item.status,
        planPrice: parseN(item.planPrice),
        planQty: parseN(item.planQty),
        amount: Math.round(amount),
        stopLoss: item.stopLoss ?? null,
        stopProfit: item.stopProfit ?? null,
        condition: item.condition || ''
      };
    });
    const buyItems = items.filter(i => i.side === 'buy');
    const sellItems = items.filter(i => i.side === 'sell');
    return {
      count: items.length,
      buyCount: buyItems.length,
      sellCount: sellItems.length,
      buyTotal: buyItems.reduce((sum, i) => sum + i.amount, 0),
      sellTotal: sellItems.reduce((sum, i) => sum + i.amount, 0),
      items
    };
  }
