  ...
  function renderPeriodReturnDashboardView(){
    ...
    const loadedAt = periodReturnState.lastLoadedAt
      ...
      : '';
    // 報價資料時間：取所有持股最新一筆報價的市場時間（哪一天幾點的資料）
    let quoteDataAt = '';
    try{
      let maxT = 0;
      ...
      if(maxT){
        ...
        quoteDataAt = `${d.getFullYear()}/${p(d.getMonth()+1)}/${p(d.getDate())} ${p(d.getHours())}:${p(d.getMinutes())}`;
      }
    }catch(e){ /* ignore */ }
    ...
    metaEl.textContent = rows.length
      ? `目前檢視：${rangeCfg.label}，可計算 ${selectedStats.available}/${selectedStats.total} 檔${quoteDataAt ? `｜報價資料時間 ${quoteDataAt}` : ''}${loadedAt ? `｜本頁計算於 ${loadedAt}` : ''}${autoRefreshText}`
      : `目前檢視：${rangeCfg.label}`;
