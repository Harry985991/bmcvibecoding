  ...
  let returnSort = { key: 'totalReturnPct', dir: 'desc' };

  // 分類排序順序（對應 getStockCategory 實際回傳值）
  const categoryOrder = { '個股': 1, '被動式ETF': 2, '主動式ETF': 3, '美國股票': 4, '債券ETF': 5, '其他': 6 };
  ...
  function getReturnSortValue(row, key){
    switch(key){
      ...
      case 'totalCost': return Number.isFinite(row.totalCost) ? row.totalCost : Number.NEGATIVE_INFINITY;
      case 'marketValue': return Number.isFinite(row.marketValue) ? row.marketValue : Number.NEGATIVE_INFINITY;
  ...
  function getCategoryBadgeClass(category){
    switch(category){
	  ...
	  let _returnOfficialRefreshToken = 0;
  ...
  function buildCashFlowEvents(){
    ...
    if(init && init.time){
      events.push({ date: cashFlowDateKey(init.time), time: init.time, amount: parseN(init.amount), type: 'initial' });
    }
    for(const acct of DB.accounts || []){
      for(const h of acct.history || []){
        if(h && (h.type === 'deposit' || h.type === 'withdraw')){
          const ds = cashFlowDateKey(h.time);
          if(!ds) continue;
          events.push({ date: ds, time: h.time || ds, amount: parseN(h.amount), type: h.type });
        }
      }
    }
